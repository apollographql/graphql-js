...
export type ExecutionArgs = {
  ...
  rootValue?: unknown;
  contextValue?: unknown;
  variableValues?: { [variable: string]: unknown } | null | undefined;
  operationName?: string | null | undefined;
  fieldResolver?: GraphQLFieldResolver<any, any> | null | undefined;
  typeResolver?: GraphQLTypeResolver<any, any> | null | undefined;
};
...
export function execute(
  ...
  rootValue?: unknown,
  contextValue?: unknown,
  variableValues?: { [variable: string]: unknown } | null,
...
function executeImpl(args: ExecutionArgs): PromiseOrValue<ExecutionResult> {
  ...
  const notErrorExeContext = exeContext as ExecutionContext;
  ...
  const data = executeOperation(
    notErrorExeContext,
    notErrorExeContext.operation,
    rootValue,
  );
  return buildResponse(notErrorExeContext, data);
}
...
export function assertValidExecutionArguments(
  schema: GraphQLSchema,
  document: DocumentNode,
  rawVariableValues: { [variable: string]: unknown } | undefined | null,
): void {
...
export function buildExecutionContext(
  ...
  fieldResolver: GraphQLFieldResolver<unknown, unknown> | undefined | null,
  typeResolver?: GraphQLTypeResolver<unknown, unknown> | undefined | null,
  ...
  for (let i = 0; i < selectionSet.selections.length; i++) {
    ...
    switch (selection.kind) {
      case Kind.FIELD: {
        ...
        const name = getFieldEntryKey(selection as FieldNode);
        if (!fields[name]) {
          fields[name] = [];
        }
        fields[name].push(selection as FieldNode);
        break;
      }
      case Kind.INLINE_FRAGMENT: {
        ...
        collectFields(
          exeContext,
          runtimeType,
          (selection as InlineFragmentNode).selectionSet,
          fields,
      ...
      case Kind.FRAGMENT_SPREAD: {
        const fragName = (selection as FragmentSpreadNode).name.value;
        if (
...
function completeListValue(
  ...
  forEach(result as any, (item: unknown, index: string | number) => {
    // No need to modify the info object containing the path,
    // since from here on it is not ever accessed by resolver functions.
    const fieldPath = addPath(path, index);
    const completedItem: any = completeValueCatchingError(
      exeContext,
      itemType,
      fieldNodes,
      info,
...
function ensureValidRuntimeType(
  runtimeTypeOrName: GraphQLObjectType | string | null | undefined,
  exeContext: ExecutionContext,
  returnType: GraphQLAbstractType,
  fieldNodes: ReadonlyArray<FieldNode>,
  info: GraphQLResolveInfo,
...
> = function(value, contextValue, info, abstractType) {
  ...
  if (isObjectLike(value) && typeof (value as any).__typename === 'string') {
