...
export function extendSchema(
  ...
  return new GraphQLSchema({
    ...
    query: getMaybeTypeByName(operationTypes.query) as any,
    mutation: getMaybeTypeByName(operationTypes.mutation) as any,
    subscription: getMaybeTypeByName(operationTypes.subscription) as any,
  ...
  function getMaybeTypeByName(
    typeName: string | undefined | null,
  ): GraphQLNamedType | undefined | null {
  ...
  function extendObjectType(type: GraphQLObjectType): GraphQLObjectType {
    ...
    return new GraphQLObjectType({
      ...
      interfaces: () => [
        ...
        // typed values, that would throw immediately while type system
        // validation with validateSchema() will produce more actionable results.
        ...interfaceNodes.map(node => astBuilder.getNamedType(node) as any),
      ],
      fields: () => ({
  ...
  function extendUnionType(type: GraphQLUnionType): GraphQLUnionType {
    ...
    return new GraphQLUnionType({
      ...
      types: () => [
        ...
        // typed values, that would throw immediately while type system
        // validation with validateSchema() will produce more actionable results.
        ...typeNodes.map(node => astBuilder.getNamedType(node) as any),
      ],
      extensionASTNodes: config.extensionASTNodes.concat(extensions),
    });
  }
