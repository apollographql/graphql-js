...
import { ASTNode, NamedTypeNode } from '../language/ast';
...
import { GraphQLSchema, assertSchema } from './schema';
import {
  GraphQLObjectType,
...
class SchemaValidationContext {
  ...
  reportError(
    message: string,
    nodes?:
      | ReadonlyArray<ASTNode | null | undefined>
      | ASTNode
      | null
      | undefined,
  ): void {
  ...
  for (const directive of context.schema.getDirectives()) {
    ...
    if (!isDirective(directive)) {
      context.reportError(
        ...
        directive && (directive as any).astNode,
      );
      continue;
    }
...
function validateName(
  ...
  node: { name: string; astNode: ASTNode | null | undefined },
): void {
...
type SDLDefinedObject<T, K> = {
  astNode: T | null | undefined;
  extensionASTNodes?: ReadonlyArray<K> | null;
};
...
>(
  object: SDLDefinedObject<T, K>,
  getter: (arg: T | K) => L | ReadonlyArray<L>,
): ReadonlyArray<L> {
...
function getUnionMemberTypeNodes(
  union: GraphQLUnionType,
  typeName: string,
): ReadonlyArray<NamedTypeNode> | null | undefined {
