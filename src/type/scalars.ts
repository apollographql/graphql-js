...
function serializeInt(value: unknown): number {
...
function serializeFloat(value: unknown): number {
  if (typeof value === 'boolean') {
    return value ? 1 : 0;
  }
...
function serializeString(rawValue: unknown): string {
...
function serializeID(rawValue: unknown): string {
  const value = serializeObject(rawValue);
...
export function isSpecifiedScalarType(type: unknown): boolean {
  return (
    isScalarType(type) &&
